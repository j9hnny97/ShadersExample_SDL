...
static std::vector<uint32_t> getBuffer(const char* filePath)
{
...
int main(int argc, char* args[])
{
    ...
    SDL_Window* window = SDL_CreateWindow("SDL2 + Vulkan", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, win_width, win_height, 0);
    ...
    auto const vertBuffer = getBuffer(makePath("shaders/vert.vert").c_str());
    VkShaderModuleCreateInfo create_info = {};
    create_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    create_info.codeSize = vertBuffer.size() * sizeof(uint32_t);
    create_info.pCode = vertBuffer.data();
    ...
    auto const fragBuffer = getBuffer(makePath("shaders/frag.frag").c_str());
    VkShaderModule fragment_shader_module = VK_NULL_HANDLE;
    create_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    create_info.codeSize = fragBuffer.size() * sizeof(uint32_t);
    create_info.pCode = fragBuffer.data();
